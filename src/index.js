...
const { startPixMonitor } = require('./mercadoPago');
...
const ticketCommand = commands.find((command) => command.data.name === 'ticket');
const ADMIN_ROLE_ID = '1485015881657483264';
const RESTRICTED_COMMANDS = new Set(['painel', 'ticket', 'estoque', 'cupom']);

const userHasAdminRole = (interaction) =>
  !!interaction.member?.roles?.cache?.has(ADMIN_ROLE_ID);

client.once(Events.ClientReady, () => {
  console.log(`Bot pronto: ${client.user.tag}`);
  startPixMonitor(client);
});
...
client.on(Events.InteractionCreate, async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (RESTRICTED_COMMANDS.has(commandName) && !userHasAdminRole(interaction)) {
      await interaction.reply({
        content: 'Você precisa do cargo de suporte para usar este comando.',
        ephemeral: true
      });
      return;
    }
  ...
  if (interaction.isStringSelectMenu()) {
    try {
      ...
      if (
        painelCommand &&
        painelCommand.panel2ProductSelectId &&
        interaction.customId === painelCommand.panel2ProductSelectId
      ) {
        await painelCommand.handlePanel2ProductSelect(interaction);
        return;
      }

      if (ticketCommand && ticketCommand.selectId && interaction.customId === ticketCommand.selectId) {
        await ticketCommand.handleSupportSelect(interaction);
        return;
      }
    ...
    if (interaction.isButton()) {
      if (ticketCommand && ticketCommand.fieldButtonIds?.includes(interaction.customId)) {
        try {
          await ticketCommand.handleFieldButton(interaction);
        } catch (error) {
          console.error('Erro ao processar o botão do Ticket:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível processar o botão do ticket.', ephemeral: true });
          }
        }
        return;
      }

      if (ticketCommand && ticketCommand.channelButtonIds?.includes(interaction.customId)) {
        try {
          await ticketCommand.handleChannelButton(interaction);
        } catch (error) {
          console.error('Erro ao processar o botão de canal do Ticket:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível processar o botão do ticket.', ephemeral: true });
          }
        }
      ...
      if (painelCommand.panel2FieldButtons?.includes(interaction.customId)) {
        try {
          await painelCommand.handlePanel2FieldButton(interaction);
        } catch (error) {
          console.error('Erro ao processar o botão do Painel 2:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível processar o botão do painel 2.', ephemeral: true });
          }
        }
        return;
      }
      ...
      if (
        painelCommand &&
        painelCommand.mercadoPagoCopyButtonId &&
        interaction.customId === painelCommand.mercadoPagoCopyButtonId
      ) {
        try {
          await painelCommand.handleMercadoPagoCopy(interaction);
        } catch (error) {
          console.error('Erro ao processar o botão de copiar código:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível copiar o código.', ephemeral: true });
          }
        }
        return;
      }
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('painel2-modal-')) {
        try {
          await painelCommand.handlePanel2ModalSubmit(interaction);
        } catch (error) {
          console.error('Erro ao processar modal do Painel 2:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível processar o formulário.', ephemeral: true });
          }
        }
      ...
      if (ticketCommand && ticketCommand.modalIds?.includes(interaction.customId)) {
        try {
          await ticketCommand.handleModalSubmit(interaction);
        } catch (error) {
          console.error('Erro ao processar modal do Ticket:', error);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({ content: 'Não foi possível processar o formulário do ticket.', ephemeral: true });
          }
        }
        return;
      }
