...
const reserveStockItems = async (tableName, quantity) => {
  ...
  const rows = await fetchStockRows(sanitizedTable, safeQuantity);
  const limitedRows = (rows || []).slice(0, safeQuantity);
  if (!limitedRows.length) {
    return { tableName: sanitizedTable, rows: [], ids: [] };
  }
  const ids = limitedRows.map((row) => row.id);
  await deleteStockRows(sanitizedTable, ids);
  return { tableName: sanitizedTable, rows: limitedRows, ids };
};

const finalizeDeliveredProducts = async () => {}; // itens já são removidos ao reservar
...
const cloneSerializableState = (state) => JSON.parse(JSON.stringify(state || {}));

const setPendingPayment = (reference, state) => {
  paymentReferences.set(reference, { state: cloneSerializableState(state), processed: false, processing: false });
  savePendingPayments();
};
...
const buildCompactPaymentReference = (state) => {
  const table = sanitizeTableName(state?.tableName || 'pedido') || 'pedido';
  const userId = String(state?.userId || '').replace(/\D/g, '');
  const channelId = String(state?.channelId || '').replace(/\D/g, '');
  const quantity = Math.max(1, Number.parseInt(state?.quantity, 10) || 1);
  const reference = `v2|${userId}|${channelId}|${quantity}|${table}`;
  return reference.slice(0, 64);
};

const parseCompactPaymentReference = (reference) => {
  if (!reference || typeof reference !== 'string') return null;
  const parts = reference.split('|');
  if (parts.length < 5 || parts[0] !== 'v2') return null;
  const [, userId, channelId, quantityRaw, tableNameRaw] = parts;
  const tableName = sanitizeTableName(tableNameRaw || '');
  const quantity = Math.max(1, Number.parseInt(quantityRaw, 10) || 1);
  if (!userId || !channelId || !tableName) return null;
  return { userId, channelId, quantity, tableName };
};

const restoreStateFromReference = (reference) => {
  const parsed = parseCompactPaymentReference(reference);
  if (!parsed) return null;
  const session = orderSessions.get(parsed.channelId) || {};
  const priceLabel = session.priceLabel || getPriceForTable(parsed.tableName) || '';
  const priceNumber = Number.isFinite(session.priceNumber) ? session.priceNumber : parsePriceNumber(priceLabel);
  return {
    userId: parsed.userId,
    channelId: parsed.channelId,
    tableName: parsed.tableName,
    quantity: parsed.quantity,
    title: session.title || parsed.tableName,
    description: session.description || null,
    imageURL: session.imageURL || null,
    color: session.color || '#2d8c00',
    priceLabel,
    priceNumber,
...
const processPaymentConfirmation = async (paymentInfo) => {
  ...
  if (!reference) return;
  let entry = paymentReferences.get(reference);
  if (!entry) {
    const restoredState = restoreStateFromReference(reference);
    if (restoredState) {
      console.log(`[MP WEBHOOK] referência ${reference} não estava em pendências; reconstruindo pelo external_reference.`);
      setPendingPayment(reference, restoredState);
      entry = paymentReferences.get(reference);
    }
  }
  if (!entry) {
  ...
  try {
    const state = cloneSerializableState(entry.state);
    state.quantity = Math.max(1, Number.parseInt(state.quantity, 10) || 1);
    const total = calculateOrderTotal(state);
    console.log(`[MP WEBHOOK] processando entrega: table=${state.tableName} quantity=${state.quantity}`);
    ...
    const itemList = reservedItems.slice(0, state.quantity).map((row) => row.item || `item-${row.id}`);
    console.log(`[MP WEBHOOK] itens separados para entrega: ${JSON.stringify(itemList)}`);
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.customId.startsWith('order:')) {
      ...
      if (action === 'gerarpix') {
        const total = calculateOrderTotal(state);
        const reference = buildCompactPaymentReference(state);
        try {
          ...
          orderSessions.set(interaction.channelId, state);
          console.log(`[MP PIX] pendência registrada para ${reference}. state=${JSON.stringify({ userId: state.userId, channelId: state.channelId, tableName: state.tableName, quantity: state.quantity })}`);
