...
const handleMenuSelect = async (interaction) => {
  ...
  const metadataResult = await getProductMetadata(selection.table);
...
const refreshProductMessage = async (interaction, selection) => {
  ...
  const product = await getProductById(selection.table, selection.id).catch(() => null);
  if (!product) {
    userSelections.delete(interaction.user.id);
    return;
  }
...
const handleModalSubmit = async (interaction) => {
  ...
  if (interaction.customId === MODAL_IDS.stock) {
    ...
    await addProductKeys(selection.table, keys, price);
    if (!selection.isMeta) {
      const productName = stripMetaName(product?.nome) || selection.table;
      const meta = await ensureMetaRow(selection.table, productName, price);
  ...
  if (interaction.customId === MODAL_IDS.price) {
    ...
    if (!selection.isMeta) {
      const productName = stripMetaName(product?.nome) || selection.table;
      const meta = await ensureMetaRow(selection.table, productName, Number(parsed.toFixed(2)));
      selection.id = meta.id;
      selection.isMeta = true;
      userSelections.set(interaction.user.id, selection);
    }
    await updateProduct(selection.table, { id: selection.id, preco: Number(parsed.toFixed(2)) });
